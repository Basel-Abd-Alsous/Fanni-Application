...
class WidgetLayoutProvider {
  static AppBar widgetLayoutAppbar(BuildContext context, String title) {
    return AppBar(
      ...
      centerTitle: true,
      leading: IconButton(onPressed: () => context.read<LayoutProviderCubit>().changeIndex(0), icon: Icon(Icons.arrow_back, color: AppColor.primaryColor)),
      actions: [
  ...
  static BlocBuilder<LayoutProviderCubit, LayoutProviderState> widgetBottomNavBar() {
    return BlocBuilder<LayoutProviderCubit, LayoutProviderState>(
      builder:
          (context, state) => BottomNavigationBar(
            ...
            items: context.read<LayoutProviderCubit>().items(context),
            onTap: context.read<LayoutProviderCubit>().changeIndex,
          ),
    );
  }
}
