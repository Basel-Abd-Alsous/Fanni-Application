...
class NotificationsController {
  ...
  static Future<void> initializeLocalNotifications() async {
    ...
    await AwesomeNotifications().initialize(
      "resource://drawable/logo_without_bg",
      [
        NotificationChannel(
  ...
  static Future<void> onActionReceivedMethodImpl(ReceivedAction receivedAction) async {
    final title = receivedAction.title; // Notification title
    final body = receivedAction.body; // Notification body (desc)
    final payload = receivedAction.payload; // Payload sent from notification
    ...
    if (!isAllowed) {
      ...
      await AwesomeNotifications().createNotification(
        content: NotificationContent(
          ...
          largeIcon: Assets.imageLogoWithoutBg,
  ...
  static void handleNotificationPayload(BuildContext context, Map<String, dynamic> payload) {
    log('⚠️ Context not available for LayoutCubit call');
  }
